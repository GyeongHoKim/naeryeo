...
| `naeryeo setup --provider=motis --motis-url=<주소>` | 자체 호스팅 엔진으로 설정 (비대화식) |
...
경로 검색은 자체 호스팅으로 외부 의존이 **사라집니다**. 다만 축이 하나 더 있습니다.

| 무엇을 하려면 | 외부 서비스가 필요한가 |
| --- | --- |
| 역·정류장 이름으로 검색 (`강남역 → 홍대입구역`) | **아니오.** MOTIS가 자기 GTFS 색인으로 이름을 직접 해석합니다. 외부 호출 0건 |
| 건물명·주소로 검색 (`아이디스 타워 → 수지구청`) | **예 (선택).** Kakao 장소 검색 키를 등록한 경우에만 동작합니다 |

즉 역·정류장 이름만 쓰신다면 어떤 외부 서비스도 호출하지 않습니다. 건물명·주소 검색은
선택 기능이며, 켜지 않으면 해당 질의만 "찾을 수 없음"으로 답합니다.
