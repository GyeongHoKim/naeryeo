...
| SC-005 두 진입점 제공자 불일치 0건 | ✅ | `newRouteFinder` 단일 경로 + T023 테스트. 추가로 `naeryeo mcp`를 stdio로 실엔진에 붙여 `find_transit_route`가 CLI와 같은 결과·같은 계약(`fareWon` 부재)을 내는 것을 확인 (research.md R13) |
